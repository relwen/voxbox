...
import 'package:voxbox/view/vocalize/vocalise_list.dart';
...
class _VocaliseScreenState extends State<VocaliseScreen> {
  ...
  void _loadData() async {
    ...
    try {
      ...
      var sectionsResponse = await VocaliseService.getSectionsFromServer();
      if (sectionsResponse.error == null) {
        setState(() {
          sections = sectionsResponse.data as List<VocaliseSection>;
          loading = false;
        });
      } else {
        // En cas d'erreur, charger depuis le stockage local
        List<VocaliseSection> localSections = await VocaliseService.getLocalSections();
        setState(() {
  ...
  void _syncSections() async {
    ...
    try {
      var response = await VocaliseService.syncAllSections();
  ...
  void _openSection(VocaliseSection section) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => VocaliseListScreen(section: section),
      ),
    );
  }
          ...
          : sections.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    VocaliseSection section = sections[index];
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            Text(
                              '${section.vocalisesCount} vocalise${section.vocalisesCount > 1 ? 's' : ''}',
                              style: const TextStyle(
                                fontSize: 12,
                                color: Colors.blue,
